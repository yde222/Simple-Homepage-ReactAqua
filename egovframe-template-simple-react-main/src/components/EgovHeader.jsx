...
import Page from "../AquaLang/AquaPage";
    ...
    return (
        ...
        <div className="header">
            <div className="inner">
                ...
                {/* <!-- PC web에서 보여지는 영역 --> */}
                <div className="user_info" style={{ display: 'flex', flexDirection: 'row' }} > 
					{/* 다국어 지원*/}
    				<IntlProvider locale={locale} messages={messages}>
     				<Page /> {/* <Page />는 학습을 위해서 임시로 불러왔음 현재 언어가 무엇인지 확인을 위해*/}
                    {/* 로그아웃 : 로그인 정보 있을때 */}
                    {sessionUserId &&
                        <>
                            <span className="person">{sessionUserName}</span>
							<FormattedMessage id="login.UserName" defaultMessage="님이, 관리자로 로그인하셨습니다."/>	
                            <button onClick={logOutHandler} className="btn">
							<FormattedMessage id="login.logOutbtn" defaultMessage="로그아웃"/>
							</button>
                        </>
                    }
                    {/* 로그인 : 로그인 정보 없을 때 */}
                    {!sessionUserId &&
						...
						<FormattedMessage id="login.logingbtn" defaultMessage="로그인"/>
						</button>
                    }
                ...
                <div className="inner">
                    ...
                    {sessionUserId &&
	                    <div className="col">
    	                    <h3>사이트관리</h3>
        	                <ul>
            	                <li><NavLink to={URL.ADMIN_SCHEDULE}>일정관리</NavLink></li>
                	            <li><NavLink to={URL.ADMIN_NOTICE_EST} className={({ isActive }) => (isActive ? "cur" : "")}>물고기 대장</NavLink></li>
                    	        <li><NavLink to={URL.ADMIN_NOTICE_CORP} className={({ isActive }) => (isActive ? "cur" : "")}>꺽기선 챠트(일,주,월,분기 샘플, 'apexcharts')</NavLink></li>
                        	    <li><NavLink to={URL.ADMIN_NOTICE_BLDN} className={({ isActive }) => (isActive ? "cur" : "")}>꺽기선 챠트(패턴 3종류, 'apexcharts')</NavLink></li>
                                <li><NavLink to={URL.ADMIN_NOTICE_LAND} className={({ isActive }) => (isActive ? "cur" : "")}>꺽기선 챠트(DB read, 'apexcharts')</NavLink></li> 
                                <li><NavLink to={URL.ADMIN_GALLERY} className={({ isActive }) => (isActive ? "cur" : "")}>막대 챠트(DB read, 'apexcharts')</NavLink></li> 
                                <li><NavLink to={URL.ADMIN_OPER} className={({ isActive }) => (isActive ? "cur" : "")}>파이 챠트(DB read, 'apexcharts')</NavLink></li> 
                                <li><NavLink to={URL.ADMIN_OPER2} className={({ isActive }) => (isActive ? "cur" : "")}>꺽기선+파이 챠트(DB read, 'apexcharts')</NavLink></li> 
			                    <li><NavLink to={URL.ADMIN_FISHTEMP1} className={({ isActive }) => (isActive ? "cur" : "")}>물고기온도(막대, 경훈) 챠트(DB read, 'apexcharts')</NavLink></li>
    			                <li><NavLink to={URL.ADMIN_FISHTEMP2} className={({ isActive }) => (isActive ? "cur" : "")}>물고기온도(꺽기선, 지은) 챠트(DB read, 'apexcharts')</NavLink></li>
                			    <li><NavLink to={URL.ADMIN_FISHTEMP3} className={({ isActive }) => (isActive ? "cur" : "")}>물고기온도(꺽기선, 평권) 챠트(DB read, 'apexcharts')</NavLink></li>
                        	</ul>
                    	</div>
                    }
