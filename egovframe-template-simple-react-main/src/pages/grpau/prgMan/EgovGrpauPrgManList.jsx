...
function EgovGrpauPrgManList(props) {
    console.group("EgovGrpauPrgManList");
    console.log("[Start] EgovGrpauPrgManList ------------------------------");
    console.log("EgovGrpauPrgManList [props] : ", props);
...
//    const bbsId = NOTICE_BLDN_ID;
    ...
    const retrieveList = useCallback( (searchCondition) => {
        ...
        const retrieveListURL = '/sym/prm/EgovProgramListManageSelect.do';
            ...
            (resp) => {
                ...
                resp.result.resultList.forEach(function (item, index) {
                    ...
                    var listIdx = resultCnt + 1 - ((currentPageNo - 1) * pageSize + index + 1);
                        console.log("--------listIdx---------"+listIdx +  "---"+ item.url);
                        console.log("--------listIdx2---------"+listIdx +  "---"+ item.progrmFileNm);
                        console.log("--------resultCnt---------"+resultCnt);
                    mutListTag.push(
                        <Link to={{pathname: URL.GRPAU_PRG_MAN_DETAIL}} 
                        state={{
        ...
        console.groupEnd("EgovGrpauPrgManList.retrieveList()");
    },[]);

//복수건 삭제
const multiDel = (checkedList) => {
    const formData = new FormData();
    const deleteBoardURL = "/sym/prm/EgovProgrmManageListDelete.do";

    var tmpList="";
    checkedList && checkedList.map((checked) => tmpList = tmpList + checked.props.state.progrmFileNm + ",");
...
useEffect(function() {
    ...
    console.log("------------------------------EgovGrpauPrgManList [End]");
    console.groupEnd("EgovGrpauPrgManList");
                            ...
                            <ul>
                                <li className="third_1 L">
                                    <label className="f_select" htmlFor="sel1">
                                        ...
                                        >
                                            <option value="1">프로그램 한글명</option>
                                        </select>
                                    </label>
                                </li>
...
export default EgovGrpauPrgManList;